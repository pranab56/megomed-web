...
const SubscriptionPlanContent = dynamic(() =>
  Promise.resolve(({ userType, messages }) => {
    ...
    const { data: apiResponse, isLoading, isError } = useGetAllPlanQuery();
    const [createSubscription, { isLoading: createSubcLoading }] = useCreateSubcriptionMutation();
    ...
    const getPlanIcon = (title) => {
      if (title?.toLowerCase().includes('pro') || title?.toLowerCase().includes('enterprise')) {
        return "⭐";
      }
      return "👤";
    };
                    ...
                    <div className="p-6 h-full flex flex-col">
                      ...
                      {plan.isBadge && (
                        <div className="text-center mb-2">
                          <Badge className="bg-green-100 text-green-800 text-xs">
                            {subscriptionPlanTranslations.featured || "Featured"}
                          </Badge>
                        </div>
                      )}
                      ...
                      <div className="text-center mb-6">
                        ...
                        {plan.isSupport && (
                          <span className="text-xs text-green-600 mt-1">
                            {subscriptionPlanTranslations.prioritySupport || "Priority Support"}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="mb-6">
                        <div className="flex items-baseline justify-center">
                          ...
                          <span className="text-lg text-gray-600 ml-1">
                            {getCurrencySymbol(plan.title)}/{getDurationText(plan.type)}
                          </span>
                        </div>
                        <p className="text-center text-sm text-gray-500 mt-2">
                          {plan.type === 'yearly'
                            ? subscriptionPlanTranslations.billedYearly || "Billed yearly"
                            : subscriptionPlanTranslations.billedMonthly || "Billed monthly"
                          }
                        </p>
                      </div>
                      ...
                      <div className="mb-4 text-center">
                        <span className="text-sm font-medium text-gray-700">
                          {formatTenderCount(plan.tenderCount)} {subscriptionPlanTranslations.tenders || "tenders"}
                        </span>
                      </div>
                      ...
                      >
                        {isCurrentPlanLoading ? (
                          <div className="flex items-center justify-center">
                            ...
                            {subscriptionPlanTranslations.status?.processing || "Processing..."}
                          </div>
                        ) : (
                          subscriptionPlanTranslations.status?.subscribe || "Subscribe Now"
                        )}
                      </Button>
                    </div>
                  </Card>
              ...
              {sortedPlans.length === 0 && (
                <div className="col-span-full text-center py-8">
                  <p className="text-gray-500">
                    {subscriptionPlanTranslations.noPlansAvailable || "No plans available for your user type."}
                  </p>
                </div>
              )}
            </div>
