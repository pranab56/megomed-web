...
import CompanyJobTenderPreview from "./companyJobPreview";
import { AvatarFallback, AvatarImage, Avatar } from "@/components/ui/avatar";
import { getImageUrl } from "@/utils/getImageUrl";
import { useCompanyJobSortingMutation } from "@/features/companyDashboard/companyDashboardApi";
import toast from "react-hot-toast";

export function CompanyAppliedJobsTender({
  category = "jobs",
  type = "applied",
  items = [],
}) {
  ...
  const [companyJobSorting, { isLoading: isSorting }] =
    useCompanyJobSortingMutation();

  const handleSorting = async (applicationId, status) => {
    try {
      const response = await companyJobSorting({
        jobID: applicationId,
        status,
      });
      if (response.success) {
    ...
    <div>
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-foreground">
          Job {type.charAt(0).toUpperCase() + type.slice(1)} Proposals
        </h3>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground mb-2">
                      Job Category: {job?.categoryName || "Uncategorized"}
                    </p>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              <p className="text-muted-foreground">
                No job proposals with status "{type}" found.
              </p>
            </CardContent>
          </Card>
        )}
...
export default CompanyAppliedJobsTender;
